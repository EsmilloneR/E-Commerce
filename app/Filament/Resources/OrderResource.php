            ...
            ->schema([
                Group::make()->schema([
                    Section::make('Order Information')->schema([
                        Select::make('user_id')->label('Customer')->relationship('user', 'name')->preload()->required()->searchable(),
                        ...
                        ToggleButtons::make('options')->options([
                            'new' => 'New',
                    ...
                    Section::make('Order Items')->schema([
                        Repeater::make('items')->relationship()->schema([
                            Select::make('product_id')->relationship('product', 'name')->searchable()->preload()->required()->distinct()->disableOptionsWhenSelectedInSiblingRepeaterItems()->reactive()->afterStateUpdated(fn($state, Set $set) => $set('unit_amount', Product::find($state)->price ?? 0))->afterStateUpdated(fn($state, Set $set) => $set('total_amount', Product::find($state)->price ?? 0))->columnSpan(4),
    ...
    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                ...
                TextColumn::make('grand_total')->numeric()->money('PHP')->sortable(),
